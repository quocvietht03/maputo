...
if ( ! function_exists( 'maputo_setup' ) ) {
	function maputo_setup() {
		/* Load textdomain */
		...
		register_nav_menus( array(
			'primary_menu'   => esc_html__( 'Primary Menu','maputo' ),
		...
		add_theme_support( 'custom-background', apply_filters( 'maputo_custom_background_args', array(
			'default-color' => 'f5f5f5',
		) ) );

		/* Add support for featured content. */
		add_theme_support( 'featured-content', array(
			'featured_content_filter' => 'maputo_get_featured_posts',
			'max_posts' => 6,
		) );
...
if (!function_exists('maputo_logo')) {
	function maputo_logo($url = '', $height = 30) {
		if(!$url){
			$url = get_template_directory_uri().'/assets/images/site-logo.png';
		}
		echo '<a href="'.home_url('/').'"><img class="logo" style="height: '.esc_attr($height).'px; width: auto;" src="'.esc_url($url).'" alt="'.esc_attr__('Logo', 'maputo').'"/></a>';
	}
}

/* Nav Menu */
if (!function_exists('maputo_nav_menu')) {
	function maputo_nav_menu($menu_slug = '', $theme_location = '', $container_class = '') {
...
/* Page title */
if (!function_exists('maputo_page_title')) {
    function maputo_page_title() {
		ob_start();
		if(is_front_page()){
			esc_html_e('Home', 'maputo');
		}elseif(is_home()){
			esc_html_e('Blog', 'maputo');
		}elseif(is_search()){
			esc_html_e('Search', 'maputo');
		}elseif(is_404()){
			esc_html_e('404', 'maputo');
		}elseif(is_page()){
			echo get_the_title();
		}elseif (is_archive()) {
			if (is_category()){
				...
				printf(__('Author: %s', 'maputo'), '<span class="vcard">' . get_the_author() . '</span>');
			}elseif (is_day()){
				printf(__('Day: %s', 'maputo'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			}elseif (is_month()){
				printf(__('Month: %s', 'maputo'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			}elseif (is_year()){
				printf(__('Year: %s', 'maputo'), '<span>' . get_the_date(get_option('date_format')) . '</span>');
			}elseif (is_tax('post_format', 'post-format-aside')){
				esc_html_e('Aside', 'maputo');
			}elseif (is_tax('post_format', 'post-format-gallery')){
				esc_html_e('Gallery', 'maputo');
			}elseif (is_tax('post_format', 'post-format-image')){
				esc_html_e('Image', 'maputo');
			}elseif (is_tax('post_format', 'post-format-video')){
				esc_html_e('Video', 'maputo');
			}elseif (is_tax('post_format', 'post-format-quote')){
				esc_html_e('Quote', 'maputo');
			}elseif (is_tax('post_format', 'post-format-link')){
				esc_html_e('Link', 'maputo');
			}elseif (is_tax('post_format', 'post-format-status')){
				esc_html_e('Status', 'maputo');
			}elseif (is_tax('post_format', 'post-format-audio')){
				esc_html_e('Audio', 'maputo');
			}elseif (is_tax('post_format', 'post-format-chat')){
				esc_html_e('Chat', 'maputo');
			}else{
				esc_html_e('Archive', 'maputo');
			}
		}else {
			echo get_the_title();
		}
...
if (!function_exists('maputo_page_breadcrumb')) {
  function maputo_page_breadcrumb($home_text = 'Home', $delimiter = '-') {
		...
		if(is_front_page()){
			echo '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Front Page', 'maputo');
		}elseif(is_home()){
			echo  '<a class="bt-home" href="' . esc_url(home_url('/')) . '">' . $home_text . '</a> <span class="bt-deli first">' . $delimiter . '</span> ' . esc_html('Blog', 'maputo');
		}else{
		...
		if(is_category()){
			...
			echo '<span class="current">' . single_cat_title(esc_html__('Archive by category: ', 'maputo'), false) . '</span>';
		}elseif (is_tag()) {
			echo '<span class="current">' . single_tag_title(esc_html__('Posts tagged: ', 'maputo'), false) . '</span>';
		}elseif(is_post_type_archive()){
			echo '<span class="current">' . post_type_archive_title( esc_html__('Archive: ', 'maputo'), false ) . '</span>';
		}elseif(is_tax()){
			echo '<span class="current">' . single_term_title(esc_html__('Archive by taxonomy: ', 'maputo'), false) . '</span>';
		}elseif(is_search()){
			echo '<span class="current">' . esc_html__('Search results for: ', 'maputo') . get_search_query() . '</span>';
		}elseif(is_day()){
        ...
        if(get_post_type() == 'product'){
			...
			echo '<span class="current">' . esc_html__('Articles posted by ', 'maputo') . $userdata->display_name . '</span>';
		}elseif ( is_404() ) {
			echo '<span class="current">' . esc_html__('Error 404', 'maputo') . '</span>';
		}

		if ( get_query_var('paged') ) {
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ' (';
				echo ' <span class="bt-deli">' . $delimiter . '</span> ' . esc_html__('Page', 'maputo') . ' ' . get_query_var('paged');
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
  }
}
...
if ( ! function_exists( 'maputo_post_nav' ) ) {
	function maputo_post_nav() {
		...
		<nav class="bt-post-nav clearfix">
			<?php
				previous_post_link('<div class="bt-post-nav--item bt-prev"><span>' . esc_html__('Previous Post', 'maputo') . '</span><h3>%link</h3></div>');
				next_post_link('<div class="bt-post-nav--item bt-next"><span>' . esc_html__('Next Post', 'maputo') . '</span><h3>%link</h3></div>');
			?>
		</nav>
		<?php
	}
...
if ( ! function_exists( 'maputo_paginate_links' ) ) {
	function maputo_paginate_links($wp_query) {
...
if ( ! function_exists( 'maputo_paging_nav' ) ) {
	function maputo_paging_nav() {
