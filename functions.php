<?php
/* Register Sidebar */
if (!function_exists('maputo_register_sidebar')) {
	function maputo_register_sidebar()
	{
		register_sidebar(array(
			'name' => esc_html__('Main Sidebar', 'maputo'),
			'id' => 'main-sidebar',
	...
	add_action('widgets_init', 'maputo_register_sidebar');
}

/* Add Support Upload Image Type SVG */
function maputo_mime_types($mimes)
{
	$mimes['svg'] = 'image/svg+xml';
	return $mimes;
}
add_filter('upload_mimes', 'maputo_mime_types');

/* Register Default Fonts */
if (!function_exists('maputo_fonts_url')) {
	function maputo_fonts_url()
	{
...
if (!function_exists('maputo_enqueue_scripts')) {
	function maputo_enqueue_scripts()
	{
		...
		/* Fonts */
		wp_enqueue_style('maputo-fonts', maputo_fonts_url(), false);
		wp_enqueue_style('maputo-main', get_template_directory_uri() . '/assets/css/main.css',  array(), false);
		wp_enqueue_style('maputo-style', get_template_directory_uri() . '/style.css',  array(), false);
		wp_enqueue_script('maputo-main', get_template_directory_uri() . '/assets/js/main.js', array('jquery'), '', true);
		if (function_exists('get_field')) {
			...
			if ($dev_mode && !empty($custom_style)) {
				wp_add_inline_style('maputo-style', $custom_style);
			}
			...
			if ($dev_mode && !empty($custom_script)) {
				wp_add_inline_script('maputo-main', $custom_script);
			}
		}
		/* Options to script */
		$js_options = array(
		...
		wp_localize_script('maputo-main', 'AJ_Options', $js_options);
		wp_enqueue_script('maputo-main');
	}
	add_action('wp_enqueue_scripts', 'maputo_enqueue_scripts');
}

/* Add Stylesheet And Script Backend */
if (!function_exists('maputo_enqueue_admin_scripts')) {
	function maputo_enqueue_admin_scripts()
	{
		wp_enqueue_script('maputo-admin-main', get_template_directory_uri() . '/assets/js/admin-main.js', array('jquery'), '', true);
		wp_enqueue_style('maputo-admin-main', get_template_directory_uri() . '/assets/css/admin-main.css', array(), false);
	}
	add_action('admin_enqueue_scripts', 'maputo_enqueue_admin_scripts');
}
...
if (function_exists('get_field')) {
	/* Orbit circle effect */
	function maputo_body_class($classes)
	{
	...
	add_filter('body_class', 'maputo_body_class');
}
